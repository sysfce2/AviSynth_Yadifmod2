...
template <>
F_INLINE VSNodeRef*
get_prop<VSNodeRef*>(const VSAPI* api, const VSMap* in, const char* name,
                     int idx, int* e)
{
    VSNodeRef* ret = api->propGetNode(in, name, idx, e);
    return ret;
}
...
constexpr int64_t get_gcd(int64_t a, int64_t b)
{
    return b == 0 ? a : get_gcd(b, a % b);
}

static inline void
set_fps(VSVideoInfo& vi, int64_t fpsnum, int64_t fpsden)
{
    int64_t gcd = get_gcd(fpsnum, fpsden);
    vi.fpsNum = fpsnum / gcd;
    vi.fpsDen = fpsden / gcd;
}
...
static F_INLINE void my_free(void* ptr) noexcept
{
#if defined(__SSE2__)
     _mm_free(ptr);
#else
...
bitblt(T* dstp, const int dstride, const T* srcp, const int sstride,
       const size_t row_size, const size_t height) noexcept
{
    if (height == 0) {
        return;
    }
